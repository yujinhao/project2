    ...
    function deletecheck(){
    	...
    	if(flag){
    		if(confirm("您确定要删除这些信息吗？"))
    			location.href="deleteScoreCheckServlet?arr="+arr;
    	}
    	else
...
<th>操作</th>
       ...
       <%
        ...
        %>
        <tr>
            <td ><input type="checkbox" name="check" value=<%=scoredto.getStudentId()+","+scoredto.getCourseId()%> /></td>
			...
			<td><%=scoredto.getSign() %></td>
			<td> <a href="updateScoreInfo.jsp?StudentId=<%=scoredto.getStudentId() %>&CourseId=<%=scoredto.getCourseId() %>"><img class="operation" src="img/update.png"></a> 
				<a href="deleteScoreServlet?StudentId=<%=scoredto.getStudentId() %>&CourseId=<%=scoredto.getCourseId() %>"><img src="img/delete.png"></a>								
			</td>
        </tr>
        <%
            }
